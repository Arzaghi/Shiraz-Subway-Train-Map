...
function isContainTrainNow(station, currentTime, direction) {
    return station.ToEhsan.filter(w => isTrainInStopState(currentTime, w + ':00', 30, 0)).length > 0
}

function isStationNextRoadContaionTrain(station, currentTime) {
    let toNext = station.NextStationToEhsan
    let time = station.ToEhsan.find(w => isTrainInStopState(currentTime, w + ':00', toNext, 30))
    if (time === undefined) return station.NextStationToEhsan
    else
        return station.NextStationToEhsan - CompareTime(currentTime, time + ':00');
}

function MapStationToEhsan(props) {
    ...
    return (
        ...
        <div>
                        ...
                        <div style={{ marginTop: '5px' }}>
                            <Tooltip TransitionComponent={Zoom} title={
                                <div style={{ direction: 'rtl' }}>
                                    ...
                                    <Typography style={{ fontFamily: 'B Nazanin', color: '#FFFFFF' }} variant='caption'>{`قطار بعدی: ${CalcNextTrainTime(StationSchedule, 'west', currentTime)}`}</Typography>
                                </div>
                            }>
            ...
            {
                station.StationID !== '16' ?
                    <div className={classes.root} >
                        <Slider
                            trackStyle={{ backgroundColor: MyTheme.palette.grey["300"] }}
                            handleStyle={{ visibility: 'hidden' }}
                            railStyle={{ backgroundColor: MyTheme.palette.secondary.main }}
                            max={station.NextStationToEhsan}
                            value={
...
MapStationToEhsan.propTypes = {
    classes: PropTypes.object.isRequired,
};

export default withStyles(styles)(MapStationToEhsan);